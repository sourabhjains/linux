...
void __init fadump_setup_param_area(void)
{
	...
	if (early_radix_enabled()) {
		...
		/*
		 * Bootloader can use memory up to MIN_RMA, so try reserving
		 * memory for passing additional parameters in memory region
		 * between MIN_RMA and RMA size.
		 */
		range_start = MIN_RMA;
		range_end = ppc64_rma_size;
	}
